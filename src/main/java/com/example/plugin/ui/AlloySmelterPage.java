...
public class AlloySmelterPage extends InteractiveCustomUIPage<AlloySmelterUiEvent> implements WindowlessPage {
    ...
    private static final short INPUT_SLOT_END =
            (short) (INPUT_SLOT_START + INPUT_SLOT_COUNT - 1);
    ...
    private void update(EnergyNodeComponent node, MachineComponent machine, boolean force) {
        ...
        changed |= updateRecipeList(update);
    ...
    private void initStaticUi(UICommandBuilder update) {
        ...
        update.set("#CrusherYield.Text", "Yield: 1x");
    ...
    private boolean updateBonusPanel(UICommandBuilder update, MachineComponent machine, ItemContainer container) {
        int tier = machine == null ? AlloySmelterConfig.MIN_TIER : AlloySmelterConfig.clampTier(machine.getTier());
        List<AlloySmelterConfig.BonusDrop> drops = AlloySmelterConfig.getByproductDrops(tier);
        ...
        update.set("#BonusHeader.Text", "Byproducts");
        boolean hasDrops = !drops.isEmpty();
        update.set("#BonusEmpty.Visible", !hasDrops);
        update.set("#BonusEmpty.Text", hasDrops ? "" : "No byproducts available.");
    ...
    private boolean updateRecipeList(UICommandBuilder update) {
        String recipeText = AlloySmelterConfig.getRecipeListText();
        if (recipeText == null) {
            recipeText = "";
        }
        if (recipeText.equals(lastRecipeKey)) {
            return false;
        }
        lastRecipeKey = recipeText;
        update.set("#RecipeList.Text", recipeText);
        return true;
    }
    ...
    private boolean handleTakeInput(Store<EntityStore> store) {
        ...
        if (machineContainer == null || machineContainer.getCapacity() <= INPUT_SLOT_START) {
            return false;
        }
        Inventory inventory = getPlayerInventoryFull(store);
        if (inventory == null) {
        ...
        for (int i = 0; i < INPUT_SLOT_COUNT; i++) {
            ...
            if (slot < 0 || slot >= machineContainer.getCapacity()) {
                continue;
            }
    ...
    private ItemStack copyStack(ItemStack stack) {
        ...
        return new ItemStack(itemId, stack.getQuantity(), stack.getMetadata());
    }
    ...
    private void handleDrag(Store<EntityStore> store, AlloySmelterUiEvent data) {
        ...
        try {
            ...
            GridType originalSourceGrid = sourceGrid;
            ...
            // Never allow dragging finished output back into machine inputs.
            if (targetGrid == GridType.INPUT && originalSourceGrid == GridType.OUTPUT) {
                return;
            }
            ...
            if (targetGrid == GridType.INPUT
                    && !AlloySmelterConfig.isValidInputItemId(sourceStack.getItemId())) {
                System.out.println("[Machinarium] DBG Drag input rejected invalid itemId=" + sourceStack.getItemId());
                return;
            }
    ...
    private boolean matchesAnyInput(ItemContainer container, String itemId) {
        ...
        if (container.getCapacity() <= INPUT_SLOT_START) {
            return false;
        }
    ...
    private SlotRef resolveSlotRef(
            ...
            PlayerIndexMode playerIndexMode) {
        if (gridType == GridType.INPUT) {
            if (machineContainer == null || slotIndex < 0 || slotIndex >= INPUT_SLOT_COUNT) {
                return null;
            }
            short inputSlot = (short) (INPUT_SLOT_START + slotIndex);
            if (inputSlot < INPUT_SLOT_START || inputSlot > INPUT_SLOT_END) {
                return null;
            }
